...
void test_BasicTryDoesNothingIfNoThrow(void) {
  ...
  Try { i += 1; }
  Catch(e) { TEST_FAIL_MESSAGE("Should Not Enter Catch If Not Thrown"); }
...
void test_ThrowAndCatchFromASubFunctionAndNoRethrowToCatchInRootFunc(void) {
  ...
  Try { HappyExceptionRethrower(0xBF); }
  Catch(e) {
    TEST_FAIL_MESSAGE(
        "Should Not Have Re-thrown Error (it should have already been caught)");
  }
...
void test_ThrowAnErrorThenEnterATryBlockFromWithinCatch_VerifyThisDoesntCorruptExceptionId(
    void) {
  CEXCEPTION_T e;
...
void test_ThrowAnErrorThenEnterATryBlockFromWithinCatch_VerifyThatEachExceptionIdIndependent(
  ...
  Catch(e1) {
    ...
    Try { HappyExceptionThrower(0x12); }
    Catch(e2) { TEST_ASSERT_EQUAL(0x12, e2); }
...
void test_CanHaveMultipleTryBlocksInASingleFunction(void) {
  ...
  Catch(e) { TEST_ASSERT_EQUAL(0x01, e); }

  Try {
    HappyExceptionThrower(0xF0);
    TEST_FAIL_MESSAGE("Should Have Thrown Exception");
  }
  Catch(e) { TEST_ASSERT_EQUAL(0xF0, e); }
}
...
void test_CanHaveNestedTryBlocksInASingleFunction_ThrowInside(void) {
  ...
  Catch(e) { TEST_FAIL_MESSAGE("Should Have Been Caught By Inside Catch"); }
...
void test_CanHaveNestedTryBlocksInASingleFunction_ThrowOutside(void) {
  ...
  Try {
    Try { i = 2; }
    Catch(e) { TEST_FAIL_MESSAGE("Should Not Be Caught Here"); }
    HappyExceptionThrower(0x01);
    TEST_FAIL_MESSAGE("Should Have Rethrown Exception");
  }
  Catch(e) { TEST_ASSERT_EQUAL(0x01, e); }
...
void test_AThrowWithoutATryCatchWillUseDefaultHandlerIfSpecified(void) {
  // Let the fallback handler know we're expecting it to get called this time,
  // so don't fail
...
void test_AThrowWithoutOutsideATryCatchWillUseDefaultHandlerEvenAfterTryCatch(
    void) {
  CEXCEPTION_T e;
  ...
  Catch(e) {
    // The entire purpose here is just to make sure things get set back to using
    // the default handler when done
  }
...
void test_AbilityToExitTryWillOnlyExitOneLevel(void) {
  ...
  Try {
    ...
    Catch(e) { TEST_FAIL_MESSAGE("Should Not Have Been Caught By Inside"); }
    i = 1;
  }
  Catch(e2) { TEST_FAIL_MESSAGE("Should Not Have Been Caught By Outside"); }
