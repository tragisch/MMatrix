...
int main();
void write_bench(long long elapsed, long long memory_used);
void test01();
void test02();
void test03();
...
*/
{
  ...
  spy_file("before", "third_party/jburkardt/examples/before_data.txt");
  spy_file("after", "third_party/jburkardt/examples/after_data.txt");
...
/*
  Purpose:
    ...
    R8MAT_UNIFORM_01_NEW fills an R8MAT with pseudorandom values scaled to
  [0,1].
    ...
    for (i = 0; i < nx; i++) {
      ...
      for (krow = 0; krow < 8; krow++) {
        ...
        for (kcol = 0; kcol < 8; kcol++) {
          jj = node[kcol];
          a[ii + jj * n] =
              a[ii + jj * n] + rho[i + j * nx] * em[krow + kcol * 8];
        }
      }
    }
  }
